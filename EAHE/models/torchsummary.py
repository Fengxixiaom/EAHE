...
def summary(model, input_size, batch_size=-1, device="cuda"):
            ...
            if isinstance(output, (list, tuple)):
                ...
                for out in output:
                    if isinstance(out, (list, tuple)):
                        ...
                        output_shape.append([-1] + list(out.size())[1:])
    ...
    for layer in summary:
        ...
        ################################################################
        # TODO: changes from
        #  `total_output += abs(np.prod(summary[layer]["output_shape"]))`
        if isinstance(summary[layer]["output_shape"][0], list):
            for out_shape in summary[layer]["output_shape"]:
                total_output += abs(np.prod(out_shape))
        else:
            total_output += abs(np.prod(summary[layer]["output_shape"]))
        ################################################################

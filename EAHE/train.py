...
from utils.general import set_logging, colorstr, init_seeds, get_latest_run, increment_path, \
    check_dataset, check_img_size, check_file, one_cycle
...
def train(opt):
    ...
    save_dir, epochs, batch_size, total_batch_size, rank = \
        Path(opt.save_dir), opt.epochs, opt.batch_size, opt.total_batch_size, opt.global_rank
    ...
    pretrained = False
    model = Model(ch=3, mode_align=mode_align).to(device)  # create
    ...
    # print("nb",dataloader[1].shape)

    
    # Process 0
    if rank in [-1, 0]:
        testloader = create_dataloader(test_path, imgsz_test, batch_size, mode=opt.mode, reg_mode=opt.reg_mode,
    ...
    compute_loss = ComputeAlignLoss(model)  # init loss class
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, imgs_list in pbar:  # batch -------------------------------------------------------------------------
            # check_fuse_input(imgs)
            imgs = imgs_list[0]
            img1_warped_mask = imgs_list[1]
            # print("imgs", img1_warped_mask.shape)
            img1_warped = imgs_list[2]
            offset = imgs_list[3]
            ni = i + nb * epoch  # number integrated batches (since train start)

            imgs, img1_warped, img1_warped_mask, offset = imgs.to(device, non_blocking=True).float() / 255.0, img1_warped.to(device, non_blocking=True) / 255.0,\
                                                          img1_warped_mask.to(device, non_blocking=True) / 255.0, offset.to(device, non_blocking=True) # uint8 to float32, 0-255 to 0.0-1.0
            ...
            with amp.autocast(enabled=cuda):
                try:
                    pred = model(imgs,img1_warped_mask,img1_warped)  # forward
                    # print("pred",pred)
                except AssertionError as e:
                    print('Warning: NaN occurs, ignore this batch.')
                    optimizer.zero_grad()
                    continue
                # loss, loss_items = compute_loss(pred, imgs, offset)  # loss scaled by batch_size
                loss, loss_items, loss_items2 = compute_loss(pred, imgs, img1_warped, img1_warped_mask, offset)  # loss scaled by batch_size
            ...
            if rank in [-1, 0]:
                ...
                # mloss = (mloss * i + loss_items2) / (i + 1)  # update mean losses
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='data/udis.yaml', help='data.yaml path')
    parser.add_argument('--hyp', type=str, default='data/hyp.align.scratch.yaml', help='hyperparameters path')
    ...
    parser.add_argument('--auto-resume', nargs='?', const=True, default=False,
                        help='auto resume last training in case training broken. e.g., runs/train/exp/weights/last.ptparameters')
